...
Page({
  ...
  submit_: function () {
    ...
    wx.request({
      url: new_http+'/hyapi/user/addexpressaddress?common_param={"consigner":"' + that.data.true_name + '","mobile":"' + that.data.mob_phone + '","address":"' + that.data.address + '","province":"' + that.data.sheng_id + '","city":"' + that.data.shi_id + '","district":"' + that.data.qu_id + '","zipcode":"' + that.data.zipcode + '","phone":"' + that.data.mob_phone +'"}&token='+that.data.key, //仅为示例，并非真实的接口地址
      header: {
        'content-type': 'application/json' // 默认值
      },
      success: function (res) {
