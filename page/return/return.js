...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.request({
      url: http +'/index.php?model=member_return&fun=get_return_list', //仅为示例，并非真实的接口地址
      data: {
        key:that.data.key,
        curpage: 1,
        page: 2
      ...
      success: function (res) {
        ...
        that.setData({
          data_list: res.data.datas.return_list
        })
  ...
  onReachBottom: function () {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        page_total = res.data.page_total
        that.setData({
          data_list: that.data.data_list.concat(res.data.datas.return_list) 
        })
